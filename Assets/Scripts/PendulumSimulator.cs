...
public class PendulumSimulator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 difference = this.transform.position - referenceObject.transform.position;

        if(difference.magnitude > syncThreshold)
        {
            SetPendulumPosition(referenceObject.transform.position);
            SetPendulumForces(referenceObject.GetComponent<Rigidbody>().velocity);
        }
    ...
    [ContextMenu("Reset Pendulum Position")]
    void SetPendulumPosition(Vector3 position)
    {
        if (this.bobStartingPositionSet)
            this.MoveBob(position);
        else
            this.PendulumInit();
    }
    ...
    // Use this to reset any built up forces
    [ContextMenu("Reset Pendulum Forces")]
    void SetPendulumForces(Vector3 velocity)
    {
